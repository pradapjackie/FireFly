...
const generateFontProperty = (fontObject) =>
  `${fontObject.fontWeight} ${fontObject.fontSize}/${fontObject.lineHeight} ${fontObject.fontFamily}`;

const generateShadowVars = (theme) =>
  theme.shadows.reduce((result, item, index, array) => {
  ...
  '@global': {
    ':root': {
      ...{
        ...
        '--error-rgb': convertHexToRGB(theme.palette.error.main),
        '--success-rgb': convertHexToRGB(theme.palette.success.main),
...
const CssVars = ({ children }) => <>{children}</>;
