...
async def raise_exception(data: Dict[str, Any], session_exception_callback: Callable):
    ...
    if message == "Session not started or terminated":
        ...
        await session_exception_callback()
    else:
        exception_class = get(error)
    raise exception_class(message, stacktrace)


async def check_response_error(*, status: int, data: Any, session_exception_callback: Callable) -> None:
    if status >= 400:
